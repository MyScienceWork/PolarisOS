...
const reorganizer = (source, duplicates) => {
    const cond = s => Utils.find_value_with_path(s, 'type'.split('.')) !== null || !s._id.startsWith('#');
    const normalizer = (s) => {
    ...
    console.log(source_name_tokens);
    console.log(duplicate_names_tokens);
...
const forger = duplicate => ({
    journal: duplicate.duplicates.map(d => d._id),
});

const replacer = (publication, duplicate) => {
    const source_id = duplicate.source._id;
    publication.journal = source_id;
    return publication;
};

const dr = new DuplicatesRemover('journal');
dr.read_from_tabular_file('/Users/Cocophotos/Desktop/doublons_revues.ined.csv')
...
//    .then((results) => {
//        console.log('s/d\t_id\tname\ttype\tkeep\tremove');
//        results.map((result) => {
//            console.log(`S\t${result.source._id}\t${result.source.name}\t${result.source.type}`);
//            result.duplicates.map((d) => {
//                console.log(`D\t${d._id}\t${d.name}\t${d.type}`);
//            });
//        });
//    })
