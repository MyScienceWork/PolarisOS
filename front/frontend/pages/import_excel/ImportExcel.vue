    ...
    <div class="container is-fluid">
        <div class="columns is-centered">
            <div class="column">
                <div class="card">
                    <div class="card-content">
                        <h4 class="has-small-top-margin title is-4">{{lang('l_excel_import')}}</h4>
                        <div>
                            <div v-if="state.import_subform_name !== '' && Object.keys(user_forms(state.import_subform_name)).length === 0" class="column">
                                <div class="columns is-centered">
                                    <loader />
                                </div>
                            </div>
                            <div v-else-if="state.import_subform_name !== ''">
                                <fform
                                        :name="state.sinks.creations.import"
                                        hasButtons="true"
                                        :hasCancelButton="false"
                                        :post_path="state.paths.creations.import"
                                        :put_path="state.paths.creations.import"
                                        :get_path="state.paths.reads.import"
                                        :get_form="state.sinks.reads.import"
                                        @form-success-reset="project_submitted"
                                >
                                <dynamic-form :form="user_forms(state.import_subform_name)" :cform="state.sinks.creations.import"/>
                                </fform>
                            </div>
                        </div>
                        <article class="message is-success" v-if="state.statuses.creations.import === 'ok'">
                            <div class="message-body">
                                {{lang('f_import_success')}}
                            </div>
                        </article>
                        <article class="message is-danger" v-if="state.statuses.creations.import === 'nok'">
                            <div class="message-body">
                                {{lang('f_import_failed')}}
                            </div>
                        </article>
                    </div>
                </div>
...
<script>
module.exports = require('./ImportExcel');
</script>
