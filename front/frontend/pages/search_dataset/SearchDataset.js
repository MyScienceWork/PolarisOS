...
module.exports = {
    ...
    data() {
        return {
            state: {
                paths: {
                    creations: {
                        search: APIRoutes.entity('dataset', 'POST', true),
                    },
                },
                sinks: {
                    reads: {
    ...
    computed: {
        query() {
            return JSON.stringify(Queries.published_dataset_search());
        },
        advanced_filters() {
            return Queries.published_dataset_search(this.user._id, this.author).$and.slice(0, 3).map(a => JSON.stringify(a));
        },
        query_search() {
