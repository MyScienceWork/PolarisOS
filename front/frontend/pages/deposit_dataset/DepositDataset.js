...
module.exports = {
    ...
    data() {
        return {
            state: {
                ...
                paths: {
                    creations: {
                        dataset: APIRoutes.entity('dataset', 'POST'), // Same path for PUT,
                        specs: 'dataset_specs',
                    },
                    reads: {
    ...
    methods: {
        reset_interface() {
            this.$store.commit(Messages.INITIALIZE, {
                form: this.state.sinks.creations.dataset,
            });
            this.$store.commit(Messages.INITIALIZE, {
        ...
        next(func, step, total) {
            ...
            this.send_information(this.state.sinks.creations.dataset);
        },
        ...
        show_success_validate(sink) {
            if (this.state.sinks.creations.dataset !== sink) {
                return;
            }
        ...
        show_success_read(sink) {
            ...
            this.handle_analyzed_file(sink, this.state.sinks.creations.dataset);
        },
        show_success(sink) {
    ...
    beforeMount() {
        ...
        case 'modify-nf':
            this.$store.state.requests.push({
                ...
                content: {
                    form: this.state.sinks.creations.dataset,
                    path: APIRoutes.entity('dataset', 'GET', false, id),
                },
            });
            this.$store.state.requests.push({
    ...
    computed: {
        ...
        path() {
            if (this.state.current_step === 0) {
                ...
                return this.state.paths.validations.dataset;
            } else if (this.in_mode('review') || this.in_mode('modify') || this.in_mode('modify-nf')) {
                return this.state.paths.creations.dataset;
            }
            return this.state.paths.creations.dataset;
        },
        current_state() {
            return this.fstate(this.state.sinks.creations.dataset);
        },
        unvalidated() {
            const form = this.fform(this.state.sinks.creations.dataset);
    ...
    watch: {
        current_state(s) {
            ...
            this.dispatch(s, this, this.state.sinks.creations.dataset);
        },
        dataset_type(nt) {
            if (!nt) {
                return;
