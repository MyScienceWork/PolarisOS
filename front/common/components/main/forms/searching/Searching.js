...
module.exports = {
    ...
    methods: {
        ...
        on_checkbox_update(key, checked) {
            this.$emit('column-checkbox-update', { name: this.props.name, key, checked });
        },
        ...
        on_checked_rows_update(checkedList, row) {
            this.$emit('table-checked-rows-update', { name: this.props.name, checkedRows: checkedList, checkedRow: row });
        },
    },
    watch: {
    },
